...
export const getStaticProps: GetStaticProps = async ({ params }) => {
  ...
  const name = await connection.run(`SELECT name, url_photo, longitude, latitude FROM counter_group WHERE slug = '${params.counter}'`)
  ...
  return {
    props: {
      details: {
        ...
        month: await data('daily', params.counter, connection, 'AND (date::date).date_add(INTERVAL 31 DAY) > current_date'),
    ...
    <>
      ...
      <div className="flex flex-wrap-reverse p-4">
        <div className="md:w-1/3 w-full pr-4">
          {details.details.map(single_counter => <DetailComponent key={single_counter.name} detail={single_counter} />)}
        </div>
